...
public class EntityZaneMob extends EntityMob
{
    ...
    public EntityZaneMob(World p_i1745_1_)
    {
        super(p_i1745_1_);
    ...
    public boolean attackEntityFrom(DamageSource p_70097_1_, float p_70097_2_)
    {
        ...
        else
        {
            ...
            SummonAidEvent summonAid = ForgeEventFactory.fireZombieSummonAid(new EntityZombie(worldObj), worldObj, i, j, k, entitylivingbase, this.getEntityAttribute(field_110186_bp).getAttributeValue());
            ...
            {
                EntityZombie EntityZaneMob;
                if (summonAid.customSummonedAid != null && summonAid.getResult() == Result.ALLOW)
                {
                    EntityZaneMob = summonAid.customSummonedAid;
                }
                else
                {
                    EntityZaneMob = new EntityZombie(this.worldObj);
                }
                ...
                for (int l = 0; l < 50; ++l)
                {
                    ...
                    {
                        EntityZaneMob.setPosition((double)i1, (double)j1, (double)k1);

                        if (this.worldObj.checkNoEntityCollision(EntityZaneMob.boundingBox) && this.worldObj.getCollidingBoundingBoxes(EntityZaneMob, EntityZaneMob.boundingBox).isEmpty() && !this.worldObj.isAnyLiquid(EntityZaneMob.boundingBox))
                        {
                            this.worldObj.spawnEntityInWorld(EntityZaneMob);
                            if (entitylivingbase != null) EntityZaneMob.setAttackTarget(entitylivingbase);
                            EntityZaneMob.onSpawnWithEgg((IEntityLivingData)null);
                            ...
                            EntityZaneMob.getEntityAttribute(field_110186_bp).applyModifier(new AttributeModifier("Zombie reinforcement callee charge", -0.05000000074505806D, 0));
                            break;
                        }
                    }
                }
    ...
    public void onKillEntity(EntityLivingBase p_70074_1_)
    {
        ...
        if ((this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) && p_70074_1_ instanceof EntityVillager)
        {
            ...
            EntityZaneMob EntityZaneMob = new EntityZaneMob(this.worldObj);
            EntityZaneMob.copyLocationAndAnglesFrom(p_70074_1_);
            this.worldObj.removeEntity(p_70074_1_);
            EntityZaneMob.onSpawnWithEgg((IEntityLivingData)null);
            EntityZaneMob.setVillager(true);

            if (p_70074_1_.isChild())
            {
                EntityZaneMob.setChild(true);
            }
    ...
    public IEntityLivingData onSpawnWithEgg(IEntityLivingData p_110161_1_)
    {
        ...
        {
            p_110161_1_1 = new EntityZaneMob.GroupData(this.worldObj.rand.nextFloat() < ForgeModContainer.zombieBabyChance, this.worldObj.rand.nextFloat() < 0.05F, null);
        }

        if (p_110161_1_1 instanceof EntityZaneMob.GroupData)
        {
            EntityZaneMob.GroupData groupdata = (EntityZaneMob.GroupData)p_110161_1_1;
